...
static char Version[] =
   "$Id: lgrind.c,v 1.2 1997/12/27 20:54:03 ballen Exp $";
#endif
 ...
 *    Oct94     Matthias Eckermann (%|ud311aa@sunmail.lrz-muenchen.de|%)
...
extern	char *bruce_start;		/* start of the current string */
   ...
   char *argv[];
{
   ...
   rest:
	 ...
	 embed = FALSE;
      ...
      if (!iseen && !embed) {
	 if (files_done == 0) {
	    printf("\\documentstyle[lgrind,fancyhead]{article}\n");
	    printf("\\pagestyle{fancy}\n");
	 }
	 printf("\\lhead[\\fancyplain{}{\\bf\\thepage}]");
	 printf("{\\fancyplain{}{\\bf %s}}\n",
		fname);
	 printf("\\rhead[\\fancyplain{}{\\bf %s}]", fname);
         printf("{\\fancyplain{}{\\bf\\thepage}}\n");
	 printf("\\cfoot{}\n");
	 if (files_done == 0) {
...
void    Internal_Help() {
   ...
   printf("usage:  lgrind  [-?] [-e] [-i] [-] [-n] [-h<header>]\n");
   printf("                [-d<description-file>] [-l<language>] <name> ...\n");
   printf("-?  show this help.\n");
   printf("-e  process a LaTeX file for embedded text.\n");
   printf("-i  process for inclusion in a LaTeX document.\n");
   printf("-   take input from standard input.\n");
   printf("-n  don't boldface keywords.\n");
   printf("-h  specifies text to go on the left side of every output page (default: none).\n");
   printf("-d  specifies the language definitions file, default: \n\t\"%s\"\n", DEFSFILE);
   printf("-l  specifies the language to use. Currently known are\n");
   printf("    C (-lc or the default),               C++ (-lc++ or -lCC),\n");
   printf("    PASCAL (-lp),      Modula-2 (-lm2),   Fortran (-lf),     RATFOR (-lr),\n");
   printf("    Yacc (-ly),        PostScript (-lps), Prolog (-lprolog), MLisp (-lmlisp),\n");
   printf("    Icon (-lI),        TeX (-ltex),       Metafont (-lmf),   Perl (-lperl),\n");
   printf("    CSH (-lcsh),       Bourne Shell(-lsh),Russell (-lrussell),\n");
   printf("    assembler (-lasm), 68000 assembler (-la68),\n");
   printf("    asm68 (-lasm68),   VMS assembler (-lvmsasm),\n");
   printf("    ISP (-lisp),       LDL (-lldl),       Linda (-llinda),   MODEL (-lm).\n");
   printf("If neither -e nor -i are specified, a complete LaTeX-file is produced.\n");
   printf("See lgrind.man or lgrind.txt for detailed explanation.\n");
}
		...
		* text on or off. */
		  ...
		  printf("%s\\lgrinde\n", &buf[2]);
	       ...
	       continue;
		       ...
		       (atendptr = expmatch(buf, l_at, &atptr,
					    (char *) NULL)) != NULL) {
	       /* This is a gross hack.  Fix it, dammit */
	    restart_at:
	       pcendptr = expmatch(cp, l_pc, &pcptr, (char *) NULL);

	       if (pcptr == NULL || atptr < pcptr) {
		  putVcp(cp, atptr-1);
		  printf("\\LGinlinetrue\\LGbegin\\lgrinde");
		  cp = atendptr;
		  atendptr = expmatch(cp, l_at, &atptr, (char *) NULL);
		  if (atptr != NULL && (pcptr == NULL || atptr < pcptr)) {
		     strncpy(temp, cp, atptr-cp);
		     temp[atptr-cp] = '\0';
		     cp = atendptr;
		     printf("\\L{\\LB{");
		     putScp(temp);
		  ...
		  continue;
		       ...
		       (ttendptr = expmatch(buf, l_tt, &ttptr,
					    (char *) NULL)) != NULL) {
	       pcendptr = expmatch(buf, l_pc, &pcptr, (char *) NULL);

	       if (pcptr == NULL || ttptr < pcptr) {
		  putVcp(buf, ttptr-1);
	       } else {
		  fputs(buf, stdout); continue;
	       }
	    } else {
	       fputs(buf, stdout);
	       continue;
...
setlang()
{
   ...
   if (i == 0) {
      ...
      fprintf(stderr,  "cannot find lgrindefs file `%s'\n", defsfile);
      exit(BADEXIT);
   }
   ...
   char *os;
{
   ...
   bruce_start = os; 	/* remember the start for @expmatch()@ */
   _escaped = FALSE;
...
"Comments may not be nested within program text within comments\n");
	 ...
	 if (code_cmnts) {
		...
		&& (txmptr == NULL || texptr < txmptr)) {
		  ...
		  fprintf(stderr,
	  "TeX text within a comment must all be on one line\n");
		  s += strlen(s);
	       }
	       continue;
	    }
		...
		&& (txmptr == NULL || tt_ptr < txmptr)) {
		  ...
		  fprintf(stderr,
	  "typewriter text within a comment must all be on one line\n\t%s", s);
		  s += strlen(s);
	       }
	       continue;
	    }
	    ...
	    if (txmptr != NULL) {
		  ...
		  fprintf(stderr,
	  "TeX math within a comment must all be on one line\n");
		  s += strlen(s);
	       }
	       continue;
	    }
