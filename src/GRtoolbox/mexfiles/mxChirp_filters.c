/* compile: mex mxChirp_filters.c -I/LIGO/GRASP/include -L/LIGO/GRASP/lib -lgrasp -L/usr/local/lib -lrecipes_c		*/
/*															*/	
/* example run: filters.m												*/
/*															*/
/* Steve Drasco 													*/
/* Summer 1998														*/
...
*prhs[])
{
	...
	float	Sample_Time, *ptrCos, *ptrSin, clscnc_time;
	int	n_phaseterms, steps_alloc, steps_filld, err_cd_sprs, i, chirp_ok;
        ...
        if (nlhs != 5)
                mexErrMsgTxt("Need 5 outputs (Max_Freq_Actual, h_c, h_s, steps_filld, clscnc_time)");
	...
	if (mxIsEmpty(prhs[9])) {
		...
		ptrCos = (float *)mxMalloc(steps_alloc*sizeof(float));
		ptrSin = (float *)mxMalloc(steps_alloc*sizeof(float));
	}
	...
	*mxGetPr(plhs[4]) = (double) clscnc_time;
	...
	for (i = 0; i < steps_filld; i++) {
		*(mxGetPr(plhs[1]) + i) = (double) ptrCos[i];
		*(mxGetPr(plhs[2]) + i) = (double) ptrSin[i];
	}
}
