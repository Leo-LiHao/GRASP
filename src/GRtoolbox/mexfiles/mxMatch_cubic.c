/* compile: mex -g -I/LIGO/GRASP/include mxMatch_cubic.c -L/LIGO/GRASP/lib -lgrasp -L/usr/local/lib -lrecipes_c		*/
/*															*/
/* example run: 		 							                                */
/*															*/
/* Steve Drasco 													*/
...
{
        float   m1ref, m2ref, matchcont, srate, flo, ftau, semimajor, semiminor, theta, mcoef[7], outval;
	...
	char	*noisefile;
        ...
        if (nlhs != 5)
                mexErrMsgTxt("Need  outputs (semimajor, semiminor, theta, mcoef, outval)");
	...
	/* call math_cubic */        
	outval = match_cubic(m1ref, m2ref, matchcont, order, srate, flo, ftau, noisefile, &semimajor, &semiminor, &theta, mcoef);
