...
static void convolve_rows_gauss __P((float *image, double *mask, long n,
                                     float *h, long width, long height));
static void convolve_cols_gauss __P((float *h, double *mask, long n, float *k,
                                     long width, long height));
static double *compute_gauss_mask_0 __P((long *num, double sigma));
static double *compute_gauss_mask_1 __P((long *num, double sigma));
static double *compute_gauss_mask_2 __P((long *num, double sigma));
...
static double *compute_gauss_mask_0(num,sigma)
  long   *num;
  double sigma;
{
  long   i, n;
...
static double *compute_gauss_mask_1(num,sigma)
  long   *num;
  double sigma;
{
  long   i, n;
...
static double *compute_gauss_mask_2(num,sigma)
  long   *num;
  double sigma;
{
  long   i, n;
...
static void convolve_rows_gauss(image,mask,n,h,width,height)
  float    *image;
  double  *mask;
  long    n;
  float   *h;
...
static void convolve_cols_gauss(h,mask,n,k,width,height)
  float   *h;
  double  *mask;
  long    n;
  float   *k;
...
void convolve_gauss(image,k,width,height,sigma,deriv_type)
  float    *image;
  float   *k;
  long    width;
  long    height;
  ...
  long    deriv_type;
{
  ...
  h = xcalloc(width*height,sizeof(float));
