...
#include "grasp.h"
static char *rcsid="$Id: multitaper.c,v 1.17 1998/06/08 16:45:27 ballen Exp $\
\n$Name: RELEASE_1_9_8 $";
...
void get_F_values(double *sri, int nf,
	     int nwin, float *Fvalue, double *b,float *cest,int imin,int imax);
/* range to search for fvalues in */
int imin,imax;

int hires(double *sqr_spec, double *el, int nwin, int num_freq, double *ares);

int jtridib(int *n, double *eps1, double *d, double *e, double *e2,
	double *lb, double *ub, int *m11, int *m,
	double *w, int *ind, int *ierr,
	double *rv4, double *rv5);

int jtinvit(int *nm, int *n, double *d, double *e, double *e2,
...
void remove_spectral_lines(float *data,int npoints,int padded_length,float nwdt,int nwin,
	int max_lines, int maxpass, int *num_removed, struct removed_lines *line_list,
	...
	if (!is_power_of_two(padded_length)) {
		GR_start_error("remove_spectral_lines()",rcsid,__FILE__,__LINE__);
		GR_report_error("Error: the padded_length must");
		GR_report_error("be an integer power of 2 <= 2^31, but is %d \n",padded_length);
		GR_end_error();
		abort();
	}
	...
	if (dof==NULL || fvalues==NULL || cestimate==NULL || workspace==NULL || 
			work_list==NULL) {
          GR_start_error("remove_spectral_lines()",rcsid,__FILE__,__LINE__);
          GR_report_error("Insufficient memory - malloc returned NULL.\n");
          GR_end_error();
          abort();
  }
	...
	*num_removed=0;
	...
	/* run through loop removing lines, ending when no additional lines removed */
	  ...
	  multitaper_spectrum(data,npoints,1 /* highres */,nwin,nwdt,4 /* normalization choice */,1.0 /* srate */,
		spec,dof,fvalues,padded_length,cestimate,outspec);
	  ...
	  if (did_remove) {
	    ...
	    for (i=0;i<npoints;i++) data[i]+=workspace[i];
	  } 
	}
	...
	if (did_remove!=0 && outspec) 
	  multitaper_spectrum(data,npoints,1 /* highres */,nwin,nwdt,4 /* normalization choice */,1.0 /* srate */,
		mtap_spec_final,dof,fvalues,padded_length,cestimate,outspec);
...
/*----------------multitaper_spectrum---------------------------*/
void
multitaper_spectrum(float *data, int npoints, int kind, int nwin, float nwdt, int inorm,
	...
	*/
	...
	if (npoints!=save_npoints || nwin!=save_nwin || nwdt!=save_nwdt) {
	 ...
	 if (lambda==NULL || tapsum==NULL || tapers==NULL) {
          GR_start_error("multitaper_spectrum()",rcsid,__FILE__,__LINE__);
          GR_report_error("Insufficient memory - realloc returned NULL.\n");
          GR_end_error();
          abort();
  	}
	...
	if (b==NULL || amu==NULL || sqr_spec==NULL || ReImSpec==NULL || 
			amp==NULL) {
          GR_start_error("multitaper_spectrum()",rcsid,__FILE__,__LINE__);
          GR_report_error("Insufficient memory - malloc returned NULL.\n");
          GR_end_error();
	...
	for (iwin=0; iwin < nwin; iwin++) {
		...
		if (2*num_freqs-3>klen || num_freqs-1+kf>num_freq_tap) {
			GR_start_error("multitaper_spectrum()",rcsid,__FILE__,__LINE__);
			GR_report_error("error in index\n");
			GR_end_error();
		}
	...
	case 1:
		...
		if (outspec) hires(sqr_spec, lambda, nwin, num_freqs, amu);
	...
	default:
		GR_start_error("multitaper_spectrum()",rcsid,__FILE__,__LINE__);
		GR_report_error("Do not recognize kind = %d\n",kind);
		GR_end_error();
		abort();
		break;
	}
...
void
multitaper_cross_spectrum(float *data1, float *data2, int npoints, int npadded, 
			  float dt, int nwin, float nwdt, double *ReImSpec12)
{
  /*
  ...
  */
  ...
  if (npoints!=save_npoints || nwin!=save_nwin || nwdt!=save_nwdt) {
    ...
    tapers=(double *)realloc((void *)tapers,(size_t)len_taps*sizeof(double));
	 if (lambda==NULL || tapsum==NULL || tapers==NULL) {
          GR_start_error("multitaper_cross_spectrum()",rcsid,__FILE__,__LINE__);
          GR_report_error("Insufficient memory - realloc returned NULL.\n");
          GR_end_error();
          abort();
  	}
  ...
  if (tap_data1==NULL || tap_data2==NULL || amp1==NULL || amp2==NULL || 
			ReImSpec1==NULL || ReImSpec2==NULL) {
          GR_start_error("multitaper_cross_spectrum()",rcsid,__FILE__,__LINE__);
          GR_report_error("Insufficient memory - malloc returned NULL.\n");
          GR_end_error();
          abort();
  ...
  for (iwin=0; iwin<nwin; iwin++) {
    ...
    for (i=0; i<num_freqs; i++) {
      ...
      ReImSpec12[2*i]=ReImSpec12[2*i] + 
	a*(ReImSpec1[2*(i+kf)]*ReImSpec2[2*(i+kf)]+
	   ReImSpec1[2*(i+kf)+1]*ReImSpec2[2*(i+kf)+1]);
      /* imag part of cross-correlation spectrum */
      ReImSpec12[2*i+1]=ReImSpec12[2*i+1] + 
	a*(ReImSpec1[2*(i+kf)]*ReImSpec2[2*(i+kf)+1]-
	   ReImSpec1[2*(i+kf)+1]*ReImSpec2[2*(i+kf)]);
    }
  }
...
/*--------------------------------------------------------*/
/*--------------------slepian_tapers--------------------------*/
void slepian_tapers(int num_points, int nwin, double *lam, 
                           float nwdt, double *tapers, double *tapsum)
{
	/*
	...
	if (nwin>=2.0*nwdt) {
		GR_start_error("slepian_tapers()",rcsid,__FILE__,__LINE__);
		GR_report_error("Warning: number of taper ");
		GR_report_error("windows %d should be LESS than 2 x NWdt = %f\n",nwin,2.0*nwdt);
		GR_report_error("Please see Percival and Walden pg 335.\n");
		GR_end_error();
	}
	...
	if (ell==NULL || diag==NULL || offdiag==NULL || offsq==NULL || evecs==NULL 
			|| ip==NULL || scratch1==NULL || scratch2 == NULL || scratch3==NULL 
	...
	jtridib(&num_points, &eps, diag, offdiag, offsq, &rlb, &rlu, &m11, &nwin, lam,
		 ip, &ierr, scratch1, scratch2);
#if DIAG1
	GR_start_error("slepian_tapers()",rcsid,__FILE__,__LINE__);
	GR_report_error( "ierr=%d rlb=%.8f rlu=%.8f\n", ierr, rlb, rlu);
	GR_report_error( "eigenvalues for the eigentapers\n");
	for (k=0; k < nwin; k++)
		GR_report_error( "%.20f ", lam[k]);
	GR_report_error( "\n");
	GR_end_error();
#endif


	jtinvit(&num_points, &num_points, diag, offdiag, offsq, &nwin, lam, ip, evecs, &ierr,
	...
	*set tolerance for iterative scheme exit
...
#if 0
	GR_start_error("adwait()",rcsid,__FILE__,__LINE__);
	GR_report_error( "test input\n adwait: %d %d %f\n", nwin, num_freq, avar);
	GR_report_error( "\n Data=\n");
	...
	for (jloop=0; jloop < num_freq; jloop++) {
		...
		for (k=0; k < 50; k++) {
			...
			das=fabs(dif);
			/*
			*GR_report_error("adwait: jloop=%d k=%d %g %g %g
		...
		/* GR_report_error("adwait: k=%d test_tol=%f\n",k, test_tol); */
		/* end  300  */
		...
		if (ares[jloop] >= 0.0)
			...
			GR_start_error("adwait()",rcsid,__FILE__,__LINE__);
			GR_report_error("Error: trying to take square root of ares[jloop=%d] = %e\n",
				jloop,ares[jloop]);
			GR_end_error();
			abort();
		}
	...
	GR_start_error("adwait()",rcsid,__FILE__,__LINE__);
	GR_report_error( "%d failed iterations\n", jitter);
	GR_end_error();
	free(spw);
	free(bias);
...
/*--------------------------------------------------------*/
void
get_F_values2(double *sr, double *si, int nf, int nwin, float *Fvalue, double *b, float *cest)
{
	/*
...
/*--------------------------------------------------------*/
void
get_F_values(double *sri, int nf, int nwin, float *Fvalue, double *b, float *cest,int imin,int imax)
{
	/*
...
hires(double *sqr_spec, double *el, int nwin, int num_freq, double *ares)
{
	...
	for (j=0; j < num_freq; j++) {
		if (ares[j] > 0.0)
			...
			GR_start_error("hires()",rcsid,__FILE__,__LINE__);
			GR_report_error("Error: trying to take square root of ares[j=%d] = %e\n",
				j,ares[j]);
			GR_end_error();
			abort();
		}	
...
int 
jtinvit(int *nm, int *n, double *d, double *e, double *e2, int *m, double *w, int *ind, double *z, int *ierr, double *rv1, double *rv2,
	 double *rv3, double *rv4, double *rv6)
{
	/* Initialized data */
...
/*--------------------------------------------------------*/
int 
jtridib(int *n, double *eps1, double *d, double *e, double *e2, double *lb, double *ub, int *m11, int *m, double *w, int *ind, int *ierr,
	 double *rv4, double *rv5)
{
	/* Initialized data */
